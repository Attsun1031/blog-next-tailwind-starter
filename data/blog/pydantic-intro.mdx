...
一方、pydantic では、`.json()` や `.parse_raw()` 関数を使うことでストレートに文字列・オブジェクト間の変換ができます。また、`datetime` のような型もうまく扱うことができます。
...
全体的に pydantic よりな比較をしてしまいましたが、対比して使うものではなく、バリデーションが必要なデータモデルを定義するには pydantic が必要でしょう。そこまで厳密でない便利なデータホルダーを定義するなら dataclass で十分でしょう。
